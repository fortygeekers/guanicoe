...
def speak(text):
    p1= subprocess.Popen(["gtts-cli","-l","es",text],stdin=None,stdout=subprocess.PIPE,stderr=None)
...
llama_in=""
llama_out=""
child = pexpect.spawn('llama.cpp/main -m llama.cpp/models/vicuna-13B-1.1-GPTQ-4bit-128g.GGML.bin --interactive-first --threads 12 --repeat_last_n 128 --keep -1 -r "User:" -f llama.cpp/prompts/miranda.txt', 
    echo=False, 
    encoding='utf-8',
    logfile=sys.stdout,
    timeout=None)
try:
    ...
    child.expect_exact('User:',timeout=15)
    llama_out = child.before
    child.expect_exact('Miranda:',timeout=15)
    llama_out = child.before
    child.expect_exact('User:',timeout=15)
    llama_out = child.before
except:
